...
function useResizableSidebar(minWidth: number = 280, maxWidth: number = 2000, defaultWidth: number = 340) {
  ...
  const resize = useCallback((e: MouseEvent) => {
    if (isResizing) {
      ...
      const viewportMaxWidth = window.innerWidth - 320;
      const effectiveMaxWidth = Math.max(minWidth, Math.min(maxWidth, viewportMaxWidth));
...
export default function CodeforcesFilter() {
  ...
  // Resizable sidebar
  const { sidebarWidth, sidebarRef, startResizing, isResizing } = useResizableSidebar(280, 680, 340);
