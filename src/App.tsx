...
// Special contest IDs that need manual categorization (from cftracker)
const OTHER_CATEGORY_CONTESTS: number[] = [2010];
const DIV12_CATEGORY_CONTESTS: number[] = [1930, 2029];

// Helper functions - Improved category detection based on cftracker
const getCategoryFromContestName = (name: string, contestId: number): string => {
  // Check special contest IDs first
  if (OTHER_CATEGORY_CONTESTS.includes(contestId)) {
    return 'Other';
  }
  if (DIV12_CATEGORY_CONTESTS.includes(contestId)) {
    return 'Div. 1 + Div. 2';
  }
  ...
  // Check for "Good Bye" and "Hello" contests (always Div 1+2)
  if (lowerName.includes('good bye') || lowerName.includes('hello ')) {
    return 'Div. 1 + Div. 2';
  }
  
  // Normalize name for div detection
  ...
  // If both Div.1 and Div.2 are present, it's Div 1+2
  if (div1Index !== -1 && div2Index !== -1) {
    return 'Div. 1 + Div. 2';
  }
  
  // Check individual divisions (order matters - check higher divs first)
...
export default function CodeforcesFilter() {
  ...
  // Build problem status map for quick lookup
  const problemStatusMap = useMemo(() => {
    ...
    problems.forEach(p => {
      ...
      map.set(key, {
        solved: solvedUsers.length > 0,
        attempted: attempted && solvedUsers.length === 0,
        users: solvedUsers
      });
    });
  ...
  useEffect(() => {
    ...
    if (filters.indexFrom || filters.indexTo) {
      result = result.filter(p => {
        const idx = p.index[0];
        const fromValid = !filters.indexFrom || idx >= filters.indexFrom;
        const toValid = !filters.indexTo || idx <= filters.indexTo;
    ...
    if (filters.ratingFrom !== '' || filters.ratingTo !== '') {
      result = result.filter(p => {
        ...
        const fromValid = filters.ratingFrom === '' || p.rating >= filters.ratingFrom;
        const toValid = filters.ratingTo === '' || p.rating <= filters.ratingTo;
        return fromValid && toValid;
      });
    }
    ...
    if (filters.solvedFrom !== '' || filters.solvedTo !== '') {
      result = result.filter(p => {
        const fromValid = filters.solvedFrom === '' || p.solvedCount >= filters.solvedFrom;
        const toValid = filters.solvedTo === '' || p.solvedCount <= filters.solvedTo;
        return fromValid && toValid;
      });
    }
    ...
    if (filters.yearFrom !== '' || filters.yearTo !== '') {
      result = result.filter(p => {
        ...
        const fromValid = filters.yearFrom === '' || year >= filters.yearFrom;
        const toValid = filters.yearTo === '' || year <= filters.yearTo;
        return fromValid && toValid;
      });
    }
    ...
    setCurrentPage(1);
  }, [problems, filters, sortField, sortDirection, trackedUsers, problemStatusMap]);
          ...
          <div className="hidden md:flex items-center gap-2 px-3 py-1.5 bg-[#0d1117] border border-[#30363d] rounded-lg">
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                className="w-24 bg-transparent text-sm text-white focus:outline-none placeholder:text-gray-500"
              />
              <button
                onClick={addUser}
                disabled={userLoading || !newHandle.trim()}
                className="p-1 hover:bg-[#21262d] rounded disabled:opacity-50 transition-colors"
              >
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        >
          ...
          <div className="h-full overflow-y-auto p-4 space-y-3">
            ...
            {/* Filter Problems Section - Codeforces Style */}
            <div className="border border-[#30363d] rounded-lg overflow-hidden bg-[#0d1117]">
              ...
              <div className="p-4 space-y-4">
                ...
                {/* Solved Count Range - Better Design */}
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2">
                    <div className="flex-1 relative group">
                      <input
                        ...
                        placeholder="min"
                        min="0"
                        className="w-full px-3 py-2.5 bg-[#0d1117] border border-[#30363d] rounded-lg text-sm text-white focus:border-blue-500 focus:outline-none transition-all placeholder:text-gray-500 text-center group-hover:border-gray-500"
                      />
                    </div>
                    <span className="text-gray-500 font-bold text-lg">—</span>
                    <div className="flex-1 relative group">
        ...
        <main className="flex-1 flex flex-col min-w-0 bg-[#0d1117]">
                    ...
                    <tr key={problemId} className="group hover:bg-[#161b22]/50 transition-colors">
                      <td className="px-2 py-3 border-b border-[#21262d]">
                        <button
                          ...
                          className={`transition-colors ${isBookmarked ? 'text-yellow-400' : 'text-gray-600 hover:text-yellow-400'}`}
                        >
                          <Bookmark className={`w-4 h-4 ${isBookmarked ? 'fill-current' : ''}`} />
                        </button>
                      </td>
                      <td className="px-2 py-3 border-b border-[#21262d]">
                        <span className="font-mono text-sm text-gray-500">{problem.contestId}</span>
                      </td>
                      <td className="px-2 py-3 border-b border-[#21262d]">
                      ...
                      <td className="px-2 py-3 border-b border-[#21262d]">
                        <div className="flex items-center gap-2">
                          ...
                          <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="text-sm font-medium text-white hover:text-blue-400 transition-colors truncate block max-w-[200px]"
                            title={problem.name}
                          >
                            {problem.name}
                          </a>
                          
                          {/* Show solved user avatars */}
                      ...
                      <td className="px-2 py-3 border-b border-[#21262d]">
                        <span className="text-sm text-gray-400 font-mono">{formatNumber(problem.solvedCount)}</span>
                      </td>
                      <td className="px-2 py-3 border-b border-[#21262d]">
                      ...
                      <td className="px-2 py-3 border-b border-[#21262d]">
                        <div className="flex flex-wrap gap-1">
                          ...
                          {problem.tags.length > 2 && (
                            <span className="text-xs px-2 py-0.5 bg-[#0d1117] rounded text-gray-500 border border-[#30363d]">
                              +{problem.tags.length - 2}
                            </span>
                          )}
                        </div>
                      ...
                      <td className="px-2 py-3 border-b border-[#21262d]">
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="p-1.5 hover:bg-[#21262d] rounded-lg text-gray-400 hover:text-white transition-colors inline-flex"
                          title="Open problem"
                        >
                          <ExternalLink className="w-4 h-4" />
                        </a>
          ...
          {totalPages > 1 && (
            <div className="h-14 border-t border-[#30363d] flex items-center justify-center gap-1.5 px-4 bg-[#161b22] flex-shrink-0">
              <button
                onClick={() => setCurrentPage(Math.max(1, currentPage - 1))}
                disabled={currentPage === 1}
                className="px-3 py-2 hover:bg-[#21262d] rounded-lg disabled:opacity-40 disabled:cursor-not-allowed transition-colors text-gray-300 flex items-center gap-1 text-sm font-medium"
              >
