...
export default function CodeforcesFilter() {
  ...
  useEffect(() => {
    ...
    const savedQuotas = localStorage.getItem('cf-quotas');
    if (savedQuotas) {
  ...
  useEffect(() => {
    localStorage.setItem('cf-quotas', JSON.stringify(quotas));
  }, [quotas]);
  ...
  const updateQuota = (id: string, text: string) => {
    setQuotas(quotas.map(q => q.id === id ? { ...q, text } : q));
  };
  ...
  return (
    <div className="h-screen bg-[#0d1117] flex flex-col overflow-hidden">
      ...
      <QuotaModal
        ...
        onSave={(text) => {
          if (editingQuota) {
            updateQuota(editingQuota.id, text);
          } else {
            addQuota(text);
          }
        }}
        onDelete={editingQuota ? () => deleteQuota(editingQuota.id) : undefined}
      />
      ...
      <nav className="h-14 bg-[#161b22] border-b border-[#30363d] flex items-center justify-between px-4 flex-shrink-0 z-50">
        ...
        {/* Middle Section - Quotas */}
        <div className="hidden lg:flex items-center gap-2 flex-1 justify-center max-w-2xl mx-4">
          {quotas.map((quota) => (
            <button
              key={quota.id}
              onClick={() => openEditQuota(quota)}
              className="group flex items-center gap-2 px-3 py-1.5 bg-[#0d1117] border border-[#30363d] hover:border-blue-500/50 rounded-lg transition-all max-w-xs"
              title="Click to edit quota"
            >
              <Target className="w-3.5 h-3.5 text-blue-400 flex-shrink-0" />
              <span className="text-sm text-gray-300 truncate">{quota.text}</span>
              <Edit3 className="w-3 h-3 text-gray-500 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0" />
            </button>
          ))}
          <button
            onClick={openAddQuota}
            className="flex items-center gap-1 px-3 py-1.5 bg-blue-500/10 border border-blue-500/30 hover:bg-blue-500/20 hover:border-blue-500/50 rounded-lg transition-all"
            title="Add new quota"
          >
            <Plus className="w-4 h-4 text-blue-400" />
            <span className="text-sm text-blue-400">Quota</span>
          </button>
        </div>
      ...
      {/* Mobile Quota Bar */}
      <div className="lg:hidden bg-[#0d1117] border-b border-[#30363d] px-4 py-2 flex items-center gap-2 overflow-x-auto">
        {quotas.map((quota) => (
          <button
            key={quota.id}
            onClick={() => openEditQuota(quota)}
            className="flex items-center gap-1.5 px-3 py-1.5 bg-[#161b22] border border-[#30363d] rounded-lg whitespace-nowrap"
          >
            <Target className="w-3 h-3 text-blue-400" />
            <span className="text-xs text-gray-300">{quota.text}</span>
          </button>
        ))}
        <button
          onClick={openAddQuota}
          className="flex items-center gap-1 px-3 py-1.5 bg-blue-500/10 border border-blue-500/30 rounded-lg whitespace-nowrap"
        >
          <Plus className="w-3 h-3 text-blue-400" />
          <span className="text-xs text-blue-400">Quota</span>
        </button>
      </div>
